  ...
  if (status === 'done') {
    return (
      <span className="flex h-4 w-4 items-center justify-center rounded-full bg-green-500/15">
        <svg className="h-2.5 w-2.5 text-green-400" fill="none" viewBox="0 0 12 12" stroke="currentColor" strokeWidth={2.5}>
...
export default function ComingSoonBanner() {
  ...
  return (
    <section
      data-build="full-banner-content"
      className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0a0a0a] p-8 md:p-10"
    >
      <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-blue-500/5 blur-2xl" />
      <div className="pointer-events-none absolute -bottom-12 -left-12 h-48 w-48 rounded-full bg-blue-500/[0.03] blur-2xl" />

      <div className="relative flex flex-col gap-8 lg:flex-row lg:gap-10">
        <div className="min-w-0 flex-1">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-white/[0.04] px-3.5 py-1.5">
            <PulseDot />
            <span className="text-[11px] font-semibold uppercase tracking-widest text-blue-400">Coming Soon</span>
          </div>

          <h1 className="mb-2 text-3xl font-semibold leading-tight text-white md:text-4xl">
            <span>mbps</span>
            <span className="text-blue-500">.pro</span>
            <span className="ml-3 text-lg font-normal text-neutral-500">is almost here</span>
          </h1>
        ...
        <div className="flex w-full shrink-0 flex-col gap-3 lg:w-52 xl:w-60">
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3.5">
            <p className="mb-2 text-[10px] uppercase tracking-widest text-neutral-700">Speed preview</p>
            <div className="mb-3 flex items-baseline gap-1">
              <span className="text-3xl font-semibold leading-none text-white tabular-nums">
            ...
            <div className="flex h-7 items-end gap-1">
              {CDN_BARS.map((bar) => (
                <div key={bar.id} className="flex flex-1 flex-col items-center gap-0.5">
                  <div className="flex h-7 w-full items-end overflow-hidden rounded-sm bg-white/[0.05]">
                    <div
            ...
            <div className="mt-1 flex justify-between">
              {CDN_BARS.map((bar) => (
                <span key={bar.id} className={`text-[9px] ${bar.isOutlier ? 'text-red-500' : 'text-neutral-700'}`}>
                  {bar.label}
                </span>
              ))}
            </div>
          ...
          <div className="rounded-xl border border-blue-500/[0.15] bg-blue-500/[0.05] p-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <p className="text-xs font-medium leading-tight text-blue-400">Variance detected</p>
                <p className="mt-0.5 text-[11px] leading-snug text-neutral-600">Fastly is 11x slower, ISP peering issue</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-white/[0.05] bg-white/[0.02] p-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[10px] font-medium text-neutral-700">Your ISP</p>
      ...
      <div className="relative mt-7 flex flex-col items-start justify-between gap-4 border-t border-white/[0.05] pt-5 sm:flex-row sm:items-center">
        <div className="flex flex-wrap items-center gap-4">
          {FOOTER_FEATURES.map((feature) => (
            <div key={feature.label} className="flex items-center gap-1.5">
              <svg className="h-3.5 w-3.5 text-neutral-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
          ...
          {!submitted ? (
            <div className="flex gap-2">
              <input
                ...
                className={`h-9 w-44 rounded-lg border bg-white/[0.04] px-3 text-sm text-white outline-none transition-colors placeholder:text-neutral-700 focus:border-blue-500/50 focus:bg-white/[0.06] ${
              ...
              <button
                ...
                className="h-9 whitespace-nowrap rounded-lg bg-blue-600 px-4 text-sm font-medium text-white transition-all hover:bg-blue-500 active:scale-95"
              >
                Notify me
              </button>
            </div>
          ) : (
            <div className="animate-fade-up text-sm text-green-400">You're on the list. We'll email you at launch.</div>
          )}
        </div>
      </div>
    </section>
